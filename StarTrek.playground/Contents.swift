...
let numOfficers = humanOfficers.count
print(numOfficers)
...
let allCaptainsAlpha = captains.map{ Offer(name: $0, ship: $1)}.sorted(by: {$0 < $1})
...
let vul = firstOfficers.filter{vulcanOfficers.contains($0.key)}.map{Officer(name: $0, ship: $1)}
print(vul.map{$0.name})


// Q7: How many these Vulcan first officers have names shorter than 5 characters?

let second = vul.filter{$0.name.count <5}
print(second.map{$0.name})
